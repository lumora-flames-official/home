...
- Individual product pages don't exist yet; `SubCategoryShowcase` is the leaf of the flow, with the catalog rails beneath it as the deepest view of a specific candle.
- **The catalog is nearly empty.** Most varieties list nothing and so render no rail — correct behaviour, not a bug. Filling them is a photography and copy task done through `/update-list`.
